...
void tar_add(FILE *tar_file, const char *file, const char *internal_name)
{
        ...
        snprintf(header.size, 12, "%011lo", end - 512 - index);
        snprintf(header.modified, 12, "%011lo ", time(0)); //Again, get this from the filesystem
        ...
        snprintf(header.checksum, 8, "%06lo ", checksum);
